...
# ── Step 3: Push Alpaca secrets ───────────────────────────────────────────────
Write-Host ""
Write-Host "  [3/5] Uploading API secrets (keys stay server-side only)..." -ForegroundColor Yellow
if ($GITHUB_TOKEN) {
    npx supabase@latest secrets set ALPACA_API_KEY=$ALPACA_KEY ALPACA_SECRET_KEY=$ALPACA_SECRET GITHUB_TOKEN=$GITHUB_TOKEN GITHUB_REPOSITORY=$GITHUB_REPO
} else {
    npx supabase@latest secrets set ALPACA_API_KEY=$ALPACA_KEY ALPACA_SECRET_KEY=$ALPACA_SECRET GITHUB_REPOSITORY=$GITHUB_REPO
}
...
# Upload index.html
npx supabase@latest storage cp $HTML_FILE ss:///site/index.html
